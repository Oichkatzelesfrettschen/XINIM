...
Current truth:
- The verified build flow is a pure CMake flow rooted in `build/<lane>/<config>`.
...
Required CMake contract:
- `cmake --preset <lane>-<config>` configures that same build tree.
- Images, logs, and bootstrapped tools are stored under that tree.
- CMake cache variables carry both the CPU lane and the low-RAM VFS profile:
  `-DXINIM_CPU_LANE=<lane> -DXINIM_VFS_PROFILE=auto|default|tiny`.
...
In progress:
- keeping the pure CMake lane docs aligned with the active build graph
...
Expanded 32-bit RCA/debug tools:
- `qemu-img`, `qemu-io`, and `qemu-nbd` for image checks and block probing
- `guestfish` and `guestmount` from `libguestfs`
- `virt-inspector` and related helpers from `guestfs-tools`
- `fsck.ext2` and `debugfs` from `e2fsprogs`
- `gdb`, `rr`, `strace`, `ltrace`, `perf`, `bpftrace`, and `valgrind`
- `llvm-objdump`, `llvm-readelf`, `llvm-nm`, `llvm-dwarfdump`, and `pahole`
- `scanelf`, `dumpelf`, and `checksec`
- AFL++, `honggfuzz`, `afl-utils`, and `radamsa`
- `radare2`, `rizin`, Ghidra, RetDec, Binwalk, Capstone, Unicorn, and Keystone

Run `bash scripts/x86_32_tooling_audit.sh` for the current host inventory.
Run `bash scripts/x86_32_full_gate.sh` for the serial 32-bit build, boot, and
image-check gate.
...
Expanded Arch / CachyOS RCA and fuzzing baseline:

BlackArch is a configured first-class repo on this host. Packages installed
from it are acceptable for local RCA, fuzzing, reversing, and stress testing;
repository code still needs normal license review before adaptation.
...
sudo apt-get install -y \
  ...
  qemu-system-x86 \
  file
python3 -m pip install --user sphinx breathe sphinx-rtd-theme
```
...
- Host packages are used for build tools only, not as kernel runtime dependency
  sources.
...
- No extra host packages are currently justified for the shared low-RAM bootfs/VFS
  lane; the resident-path design stays freestanding by intent.
...
- Those scripts are not part of the canonical CMake build flow.
...
Known debt:
- `-Wconversion` and `-Wsign-conversion` are treated as errors when
  `XINIM_ENABLE_WERROR=ON`.
- Any remaining conversion debt must be fixed at the call site instead of
  demoted in the warning policy.
...
```bash
ctest --test-dir build/x86_64/Debug -L "boot|kshell" --output-on-failure

env -u CFLAGS -u CXXFLAGS -u LDFLAGS cmake --preset i486-standalone
env -u CFLAGS -u CXXFLAGS -u LDFLAGS cmake --build build/i486/Debug --target xinim_i486_image
