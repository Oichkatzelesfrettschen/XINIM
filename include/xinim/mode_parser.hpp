/**
 * @file mode_parser.hpp
 * @brief Utility for parsing symbolic and octal file permission strings.
 */
#pragma once
...
class ModeParser {
  public:
     ...
     * @param current_perms The current permissions of the file/directory, needed for symbolic
     * modes.
     * @return A ParseResult containing the new permissions on success, or an error string on
     * failure.
     */
    ParseResult parse(std::string_view mode_str, std::filesystem::perms current_perms) const {
        if (mode_str.empty()) {
  ...
  private:
    ParseResult parse_octal(std::string_view mode_str) const {
        ...
        if (mode_value > 07777) {
            return std::unexpected(std::string("Octal mode value too large: '") +
                                   std::string(mode_str) + "' (max 07777)");
        }
    ...
    std::filesystem::perms get_permission_char_mask(char perm_char) const {
        switch (perm_char) {
        case 'r':
            return std::filesystem::perms::owner_read | std::filesystem::perms::group_read |
                   std::filesystem::perms::others_read;
        case 'w':
            return std::filesystem::perms::owner_write | std::filesystem::perms::group_write |
                   std::filesystem::perms::others_write;
        case 'x':
            return std::filesystem::perms::owner_exec | std::filesystem::perms::group_exec |
                   std::filesystem::perms::others_exec;
        case 's':
            return std::filesystem::perms::set_uid |
                   std::filesystem::perms::set_gid; // 's' potentially affects both
        case 't':
            return std::filesystem::perms::sticky_bit;
        ...
        default:
            return std::filesystem::perms::unknown; // Mark as invalid
        }
    }

    ParseResult parse_symbolic_clause(
        std::string_view clause,
        std::filesystem::perms /*current_perms_for_context*/, // current_perms not directly used
                                                              // here for +,-,= with r,w,x,s,t;
                                                              // needed for 'X'
        ...
        if (who_str.empty()) { // Default to 'a' (umask is not handled by parser)
            who_target_mask = std::filesystem::perms::owner_all |
            ...
            for (char c : who_str) {
                ...
                case 'u':
                    who_target_mask |=
                        (std::filesystem::perms::owner_all | std::filesystem::perms::set_uid);
                    break;
                case 'g':
                    who_target_mask |=
                        (std::filesystem::perms::group_all | std::filesystem::perms::set_gid);
                    break;
                case 'o':
                    who_target_mask |=
                        (std::filesystem::perms::others_all | std::filesystem::perms::sticky_bit);
                    break; // 'o' can affect sticky bit for some interpretations
                case 'a':
                    who_target_mask |=
                        (std::filesystem::perms::all | std::filesystem::perms::set_uid |
                         std::filesystem::perms::set_gid | std::filesystem::perms::sticky_bit);
                    break;
                default:
                    return std::unexpected(std::string("Invalid 'who' character '") + c +
                                           "' in symbolic mode: '" + std::string(clause) + "'");
                }
            }
        }
        ...
        if (what_str.empty() && op != '=') { // e.g. "u+", "g-" - perms_from_what_str remains none
            ...
            for (char c : what_str) {
                ...
                if (p_bit == std::filesystem::perms::unknown) {
                    return std::unexpected(std::string("Invalid permission character '") + c +
                                           "' in symbolic mode: '" + std::string(clause) + "'");
                }
        ...
        // For example, if what_str is "rwx" (which translates to perms_owner_all | perms_group_all
        // | perms_others_all) and who_str is "u", then effective_perms_to_change should only be
        // perms_owner_all.
        ...
        default: // Should not be reached if op_pos check is good, but as a safeguard.
            return std::unexpected(std::string("Invalid operator '") + op +
                                   "' in symbolic mode: '" + std::string(clause) + "'");
        }
    ...
    ParseResult parse_symbolic(std::string_view mode_str,
        ...
        while (start < mode_str.length()) {
            ...
            auto clause_parse_result =
                parse_symbolic_clause(clause_str, current_perms, calculated_perms);
