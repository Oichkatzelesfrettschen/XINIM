/**
 * @file fs_error.hpp
 * @brief Error category mapping Minix filesystem codes to
 *
 * `std::error_code`.
 */
#pragma once
...
/// Error category translating Minix filesystem error codes.
class MinixFsErrorCategory : public std::error_category {
  public:
    /// Name of the error category.
    const char *name() const noexcept override { return "minix_fs"; }

    /// Convert an error condition into a human-readable message.
    std::string message(int condition) const override {
        ...
        case ErrorCode::EOF_ERROR: // Note: EOF_ERROR is -104, not in main block
            ...
            // Kernel-generated errors (these might need their own category if treated distinctly
            // from FS errors with same value) For now, providing generic messages if their values
            // overlap with FS errors. Example: EPERM and E_BAD_DEST are both -1. Context is key.
            // This category is "minix_fs", so these might be less relevant here or need
            // namespacing. For the purpose of this exercise, I'll only list a few that don't
            // directly overlap with common FS ones shown above. case ErrorCode::E_TRY_AGAIN:
            // (overlaps EAGAIN)
