/**
 * @file filesystem.hpp
 * @brief High-level filesystem operations with contextual execution modes.
 */
#pragma once
...
typedef unsigned short mode_t_placeholder; // Using a placeholder suffix to avoid conflict if mode_t
                                           // is already defined somehow
...
/// Retrieve extended status information for a filesystem entry.
...
create_directory(const std::filesystem::path &path,
                 std::filesystem::perms prms = std::filesystem::perms::all,
                 const operation_context &ctx = {});

/// Recursively create directories ensuring the final path exists.
std::expected<void, std::error_code>
create_directories(const std::filesystem::path &path,
                   std::filesystem::perms prms_for_final_dir = std::filesystem::perms::all,
                   const operation_context &ctx = {});

/// Change file permissions to a caller-specified set.
std::expected<void, std::error_code> change_permissions(const std::filesystem::path &path,
                                                        std::filesystem::perms perms,
                                                        const operation_context &ctx = {});

/// Change ownership of a file to the given user and group.
...
read_symlink(const std::filesystem::path &path, const operation_context &ctx = {});

/// Update access and modification timestamps of a file.
std::expected<void, std::error_code>
set_file_times(const std::filesystem::path &path,
...
create_file(const std::filesystem::path &path,
            std::filesystem::perms prms = static_cast<std::filesystem::perms>(0666),
            bool fail_if_exists = false, const operation_context &ctx = {});
...
/// Copy a symbolic link by replicating its target.
std::expected<void, std::error_code> copy_symlink(const std::filesystem::path &from,
                                                  const std::filesystem::path &to,
                                                  const operation_context &ctx = {});

/// General recursive copy implementation.
std::expected<void, std::error_code> copy(const std::filesystem::path &from,
                                          const std::filesystem::path &to,
                                          std::filesystem::copy_options options,
                                          const operation_context &ctx = {});

/// Set an extended filesystem attribute.
std::expected<void, std::error_code> set_extended_attr(const std::filesystem::path &path,
                                                       std::string_view name,
                                                       std::span<const std::byte> value,
                                                       const operation_context &ctx = {});

// Other xattr functions like get_extended_attr, list_extended_attrs, remove_extended_attr would go
// here.
