...
Signal sig_nr; /* signal to be sent in range 1 - 16 */
{
    /* A task wants to send a signal to a process.   Examples of such tasks are:
     *   TTY wanting to cause Signal::SIGINT upon getting a DEL
     *   CLOCK wanting to cause Signal::SIGALRM when timer expires
    ...
    rp->p_pending |= 1 << (static_cast<int>(sig_nr) - 1);
...
int proc_nr; /* MM_PROC_NR or FS_PROC_NR */
{
     ...
     * (e.g. clock task for Signal::SIGALRM), cause_sig() is called to set a bit in the
