...
void p_dmp() noexcept { // K&R -> modern C++, added noexcept
    ...
    dst = umap(proc_addr(SYSTASK), D, reinterpret_cast<std::size_t>(nbuff), static_cast<std::size_t>(NSIZE));
    ...
    for (rp = &proc[0]; rp < &proc[NR_PROCS + NR_TASKS]; rp++) {
        ...
        last = rp->p_map[S].mem_phys + rp->p_map[S].mem_len; // mem_phys is uint64_t, mem_len is vir_clicks (std::size_t)
        ...
        base = (rp->p_map[T].mem_phys + rp->p_map[T].mem_len) * CLICK_SIZE / 1024; // Example, needs precise def of what base/limit mean
        limit = (rp->p_map[S].mem_phys + rp->p_map[S].mem_len) * CLICK_SIZE / 1024; // This is just an example, original logic for base/limit was complex
        ...
        printf(" %4d %4" PRIxPTR " %4" PRIxPTR " %4x %6" PRId64 " %7" PRId64 "  %3zuK %3zuK  ",
               ...
               rp->user_time,
               rp->sys_time,
               (rp->p_map[D].mem_vir + rp->p_map[D].mem_len) * CLICK_SIZE / 1024, // Base of stack in K (approx)
...
void map_dmp() noexcept { // K&R -> modern C++, added noexcept
    ...
    for (rp = proc_addr(0); rp < proc_addr(NR_PROCS); rp++) {
        ...
        // size_k = ((rp->p_map[S].mem_phys + rp->p_map[S].mem_len - rp->p_map[T].mem_phys) * CLICK_SIZE) / 1024;
        // The above calculation for size_k is a bit off. Let's just print segment details.

        prname(rp - proc); // This will print task names too if loop is from proc[0]
                           // If only user procs: for (rp = proc_addr(0); rp < proc_addr(NR_PROCS); rp++)
                           // and prname(rp - proc - NR_TASKS);
...
char *nayme[] = {"PRINTR", "TTY   ", "WINCHE", "FLOPPY", "RAMDSK", "CLOCK ",
                 "SYS   ", "HARDWR", "MM    ", "FS    ", "INIT  "}; // Assuming INIT is at index NR_TASKS + 0 for user procs
...
static void set_name(int proc_nr, char *ptr) noexcept { // Added noexcept
    ...
    // umap takes (proc*, int, std::size_t, std::size_t) returns uint64_t
    // ptr is char*.
    src = umap(proc_addr(proc_nr), D, reinterpret_cast<std::size_t>(ptr + 2), static_cast<std::size_t>(2));
    if (src == 0)
        return;
    // &vargv is int*.
    dst = umap(proc_addr(SYSTASK), D, reinterpret_cast<std::size_t>(&vargv), static_cast<std::size_t>(2));
    // phys_copy takes (uint64_t, uint64_t, uint64_t)
