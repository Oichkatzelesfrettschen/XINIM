...
def lane_names_from_args(args: argparse.Namespace, manifest: dict) -> list[str]:
    if args.all_x86_32:
        return [
            name for name, meta in manifest.items() if meta.get("family") == "x86_32"
        ]
    if args.lane:
        return args.lane
...
def test_lane(
    build_dir: pathlib.Path, lane_meta: dict, test_kinds: Iterable[str], dry_run: bool
) -> int:
    test_name_map = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--lane", action="append", help="lane name to prepare, test, or launch"
    )
    parser.add_argument(
        "--all-x86-32",
        action="store_true",
        help="run across all registered 32-bit lanes",
    )
    parser.add_argument(
        "--list-lanes", action="store_true", help="print registered lanes and exit"
    )
    parser.add_argument(
        "--prepare",
        action="store_true",
        help="build the lane image target before running",
    )
    parser.add_argument(
        "--test",
        action="append",
    ...
    parser.add_argument(
        "--launch", action="store_true", help="launch QEMU for the selected lane"
    )
    parser.add_argument(
        "--boot-mode",
        choices=("disk", "iso"),
    ...
    parser.add_argument(
        "--cpu-override", help="override the lane's default QEMU CPU model"
    )
    parser.add_argument(
        "--memory-override", help="override the lane's default guest RAM size"
    )
    parser.add_argument(
...
def main() -> int:
    ...
    manifest_path = (
        ...
        if args.manifest
        else default_manifest_path(build_dir)
    )
