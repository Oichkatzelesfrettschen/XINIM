...
copy1(file_name) char *file_name;
{
...
copy2(num, file_name) int num; /* which program is this (0 - 4) */
char *file_name;               /* file to open */
{
    ...
    if (sepid && ((text_bytes % 16) != 0)) {
        pexit("separate I & D but text size not multiple of 16 bytes.  File: ", file_name);
    }
    ...
    if (num < FSCK) {
        printf("%s  text=%5u  data=%5u  bss=%5u  tot=%5u  hex=%4x  %s\n", name[num], text_bytes,
               data_bytes, bss_bytes, tot_bytes, tot_bytes,
               (sizes[num].sep_id ? "Separate I & D" : ""));
    }
...
read_header(fd, sepid, text_bytes, data_bytes, bss_bytes, file_name) int fd, *sepid;
...
wr_out(buffer, bytes) char buffer[READ_UNIT];
int bytes;
{
...
clear_buf() {
    ...
    for (p = buf; p < &buf[SECTOR_SIZE]; p++)
        *p = 0;
    buf_bytes = 0;
    cur_sector++;
}

patch1(all_size) long all_size;
{
...
put_byte(offset, byte_value) long offset;
int byte_value;
{
...
read_block(blk, buff) int blk;
char buff[SECTOR_SIZE];
{
    lseek(image, (long)SECTOR_SIZE * (long)blk, 0);
    if (read(image, buff, SECTOR_SIZE) != SECTOR_SIZE)
        pexit("block read error", "");
}

write_block(blk, buff) int blk;
char buff[SECTOR_SIZE];
{
    lseek(image, (long)SECTOR_SIZE * (long)blk, 0);
    if (write(image, buff, SECTOR_SIZE) != SECTOR_SIZE)
...
read_block(blocknr, user) int blocknr;
...
write_block(blocknr, user) int blocknr;
char user[SECTOR_SIZE];
{
...
dexit(s, drive, sectnum, err) int sectnum, err, drive;
char *s;
{
    extern char *derrtab[];
...
create_image(s) char *s;
{
    char kbstr[10];
    if (s[1] != ':')
        pexit("wrong drive name (dos): ", s);
    drive = (s[0] & ~32) - 'A';
    if (drive < 0 || drive > 32)
        pexit("no such drive: ", s);
    printf("Put a blank, formatted diskette in drive %s\nHit return when ready", s);
    gets(kbstr, 10);
    puts("");
}

char *derrtab[14] = {"no error",
                     "disk is read-only",
                     "unknown unit",
                     "device not ready",
                     "bad command",
