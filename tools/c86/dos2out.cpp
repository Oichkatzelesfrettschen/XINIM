...
#define IS_EXE                                                                                     \
    0x5A4D /* valid linker signature; note that                                                    \
...
char *argv[];
{
    ...
    t_size <<= 4;
    load_size = (d_ptr->nr_of_pages - 2) * PH_SECTSIZE + d_ptr->last_page_size;
    ...
    a_ptr->a_totb = a_ptr->a_text + a_ptr->a_bss + a_ptr->a_data + d_ptr->sp_reg;
    ...
    while (block_cnt--) {
        ...
        if ((in_cnt = read(inf, inbuf, PH_SECTSIZE)) != PH_SECTSIZE)
            if (block_cnt || (!block_cnt && in_cnt < lastp_cnt)) {
                printf("read %d bytes, should be %d - abort\n", in_cnt,
                       (block_cnt ? PH_SECTSIZE : lastp_cnt));
                exit(2);
            };
        i = (!block_cnt && lastp_cnt < NOT_A_HDR_LEN ? lastp_cnt : NOT_A_HDR_LEN);
        dptr = &outbuf[A_HDR_LEN];
        sptr = inbuf;
        while (i--)
            *dptr++ = *sptr++;

        i = (!block_cnt && lastp_cnt < NOT_A_HDR_LEN ? lastp_cnt + A_HDR_LEN : PH_SECTSIZE);
        if ((out_cnt = write(outf, outbuf, i)) != i) {
            printf("wrote %d bytes, should be %d - abort\n", out_cnt, i);
            exit(2);
        }

        i = (i < PH_SECTSIZE ? 0 : (!block_cnt ? lastp_cnt - NOT_A_HDR_LEN : A_HDR_LEN));
        dptr = outbuf;
        while (i--)
            *dptr++ = *sptr++;
    }
