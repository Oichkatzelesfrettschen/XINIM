 ...
 * system call, or from the keyboard (Signal::SIGINT) or from the clock (Signal::SIGALRM).
 ...
 *   do_ksig:	accept a signal originating in the kernel (e.g., Signal::SIGINT)
...
PUBLIC int do_signal() {
    ...
    if (sig == static_cast<int>(Signal::SIGKILL))
        return (OK);       /* Signal::SIGKILL may not ignored/caught */
...
PUBLIC int do_ksig() {
     ...
     * also uses this mechanism to signal writing on broken pipes (Signal::SIGPIPE).
     */
    ...
    if (sig_map == 1 << (static_cast<int>(Signal::STACK_FAULT) - 1)) {
        stack_fault(proc_nr);
        return (OK);
    }
     ...
     * and pass them to MM in one blow.  Thus loop on the bit map. For Signal::SIGINT
     * and Signal::SIGQUIT, use proc_id 0, since multiple processes may have to signalled.
     */
    for (i = 0; i < NR_SIGS; i++) {
        id =
            (i + 1 == static_cast<int>(Signal::SIGINT) || i + 1 == static_cast<int>(Signal::SIGQUIT)
                 ? 0
                 : proc_id);
        if ((sig_map >> i) & 1)
...
Signal sig_nr; /* which signal to send (1-16) */
uid send_uid;  /* identity of process sending the signal */
{
    /* Check to see if it is possible to send a signal.  The signal may have to be
     * sent to a group of processes.  This routine is invoked by the KILL system
     * call, and also when the kernel catches a DEL or other signal. Signal::SIGALRM too.
     */
    ...
    if (static_cast<int>(sig_nr) < 1 || static_cast<int>(sig_nr) > NR_SIGS)
        return (ErrorCode::EINVAL);
    count = 0; /* count # of signals sent */
    mask = 1 << (static_cast<int>(sig_nr) - 1);
...
Signal sig_nr;              /* signal to send to process (1-16) */
{
    ...
    core_file = (core_bits >> (static_cast<int>(sig_nr) - 1)) & 1;
