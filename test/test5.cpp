...
test50() {
    ...
    if (signal(1, SIG_DFL) < 0)
        e(4);
    if (signal(10, SIG_DFL) < 0)
        e(5);
}

parent(childpid) int childpid;
{
    int i;

    for (i = 0; i < 3; i++) {
        if (kill(childpid, 1) < 0)
            e(6);
        while (parsigs == 0)
            ;
        parsigs--;
...
test55() {
     ...
     * get ErrorCode::EINTR as error status.  Check that.
     */
    int n, j, i;
    ...
    if (n = fork()) {
        ...
        if (wait(&n) < 0)
            e(27);
        if (signal(8, SIG_DFL) < 0)
            e(28);
    } else {
        j = pause();
        if (errno != ErrorCode::EINTR && -errno != ErrorCode::EINTR)
            e(29);
        exit(0);
    }
}
