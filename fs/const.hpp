...
// #define NR_BUF_HASH 32                /* size of buf hash table; MUST BE POWER OF 2*/ // Replaced by kNrBufHash
...
inline constexpr bit_nr NO_BIT = static_cast<bit_nr>(0); /* returned by alloc_bit() to signal failure */
...
inline constexpr inode_nr ROOT_INODE = static_cast<inode_nr>(1);  /* inode number for root directory */
...
inline constexpr std::size_t PIPE_SIZE = static_cast<std::size_t>(NR_DZONE_NUM) * BLOCK_SIZE;        /* pipe size in bytes*/
