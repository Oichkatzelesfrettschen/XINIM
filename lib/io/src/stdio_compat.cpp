...
Stream* get_stream(FILE* file) {
    if (file == stdin) return &minix::io::stdin();
    if (file == stdout) return &minix::io::stdout();
    if (file == stderr) return &minix::io::stderr();
    std::lock_guard<std::mutex> lock(map_mutex);
...
FILE* fopen_compat(const char* path, const char* mode) {
    ...
    for (const char* p = mode; *p; ++p) {
        switch (*p) {
            ...
            case 'w': write = true; open_mode = open_mode | OpenMode::create | OpenMode::truncate; break;
            case 'a': append = true; write = true; open_mode = open_mode | OpenMode::append; break;
...
size_t fwrite_compat(const void* ptr, size_t size, size_t nmemb, FILE* fp) {
    ...
    if (!stream) return 0;
    size_t bytes = size * nmemb;
    auto result = stream->write(std::span<const std::byte>(static_cast<const std::byte*>(ptr), bytes));
    return result ? (*result.value / size) : 0;
}
...
int fprintf_compat(FILE* fp, const char* format, ...) {
    ...
    va_end(args);
    if (n < 0) return -1;
    auto wr = stream->write(std::span<const std::byte>(reinterpret_cast<const std::byte*>(buffer), static_cast<size_t>(n)));
    return wr ? n : -1;
}
