# 0 "userland/shell/mksh/integration/xinim_syscalls.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 0 "<command-line>" 2
# 1 "userland/shell/mksh/integration/xinim_syscalls.c"
# 9 "userland/shell/mksh/integration/xinim_syscalls.c"
...
# 10 "userland/shell/mksh/integration/xinim_syscalls.c" 2
...
# 53 "/usr/include/x86_64-linux-gnu/bits/unistd.h" 3 4
extern __inline __attribute__ ((__always_inline__)) __attribute__ ((__gnu_inline__)) __attribute__ ((__artificial__)) __attribute__ ((__warn_unused_result__)) ssize_t
...
# 11 "userland/shell/mksh/integration/xinim_syscalls.c" 2
...
# 13 "userland/shell/mksh/integration/xinim_syscalls.c" 2
...
# 14 "userland/shell/mksh/integration/xinim_syscalls.c" 2
...
# 16 "userland/shell/mksh/integration/xinim_syscalls.c" 2
# 43 "userland/shell/mksh/integration/xinim_syscalls.c"
...
# 57 "userland/shell/mksh/integration/xinim_syscalls.c" 3 4
       (*__errno_location ())
# 57 "userland/shell/mksh/integration/xinim_syscalls.c"
             = (int)-ret;
        return -1;
    }
    return ret;
...
int xinim_open(const char *pathname, int flags, mode_t mode) {
    return (int)xinim_syscall(5, (long)pathname, flags, mode, 0, 0);
}

ssize_t xinim_read(int fd, void *buf, size_t count) {
    return (ssize_t)xinim_syscall(6, fd, (long)buf, count, 0, 0);
}

ssize_t xinim_write(int fd, const void *buf, size_t count) {
    return (ssize_t)xinim_syscall(7, fd, (long)buf, count, 0, 0);
}

int xinim_close(int fd) {
    return (int)xinim_syscall(8, fd, 0, 0, 0, 0);
}

int xinim_dup(int oldfd) {
    return (int)xinim_syscall(9, oldfd, 0, 0, 0, 0);
}

int xinim_dup2(int oldfd, int newfd) {
    return (int)xinim_syscall(10, oldfd, newfd, 0, 0, 0);
}

int xinim_pipe(int pipefd[2]) {
    return (int)xinim_syscall(11, (long)pipefd, 0, 0, 0, 0);
}



int xinim_kill(pid_t pid, int sig) {
    return (int)xinim_syscall(12, pid, sig, 0, 0, 0);
}

typedef void (*sighandler_t)(int);

sighandler_t xinim_signal(int signum, sighandler_t handler) {
    long ret = xinim_syscall(13, signum, (long)handler, 0, 0, 0);
...
pid_t xinim_getpid(void) {
    return (pid_t)xinim_syscall(14, 0, 0, 0, 0, 0);
}

pid_t xinim_getppid(void) {
    return (pid_t)xinim_syscall(15, 0, 0, 0, 0, 0);
}



char *xinim_getenv(const char *name) {
    return (char *)xinim_syscall(16, (long)name, 0, 0, 0, 0);
}

int xinim_setenv(const char *name, const char *value, int overwrite) {
    return (int)xinim_syscall(17, (long)name,
                              (long)value, overwrite, 0, 0);
}

int xinim_unsetenv(const char *name) {
    return (int)xinim_syscall(17, (long)name, 0, 1, 0, 0);
}
