...
default 32-bit lane is Clang with `-m32`. `cross-elf` is an explicit
32-bit alternative that still uses the same Clang and Clang++ drivers with
`--target=<triple>`; only the selected `<triple>-*` binutils provide
archiving, linking, and ELF inspection. The x86_64 dietlibc, pinned mksh, and
32-bit utility recipes use the same Clang drivers and Clang compiler-rt
builtins. GCC drivers, libgcc, and libstdc++ are not valid project build
inputs. Vendor C uses the explicit GNU C11 profile, while Issue 7 C tests use
strict C99 and their documented feature-test macros.
