...
int readc() {
    ...
    if (e.iop >= iostack || multiline && eofc++ < 3)
        return (0);
    leave();
    /* NOTREACHED */
}

void ioecho(c) char c;
{
    if (flag['v'])
        write(2, &c, sizeof c);
}

void pushio(arg, fn) struct ioarg arg;
int (*fn)();
{
...
void prn(u) unsigned u;
{
    prs(itoa(u, 0));
}

void closef(i) register i;
{
    if (i > 2)
        close(i);
}
...
static struct block *readhere(s, ec)
register char *s;
{
...
static savec(c, bp) register struct block *bp;
{
    register char *np;
...
herein(bp, xdoll) struct block *bp;
{
    register tf;
    char tname[50];
    static int inc;
...
scraphere() { herelist = NULL; }

char *memcpy(ato, from, nb)
