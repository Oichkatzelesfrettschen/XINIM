...
#define block_size()                                                                               \
    (int)((convert(header.member.m_size, LONG_TYPE) + (long)BLOCK_SIZE - 1) / (long)BLOCK_SIZE)
    ...
    while (get_header()) {
        ...
        if (ext_fl) {
            ...
            if (header.member.m_linked == '1')
                string_print(NIL_PTR, "linked to %s", header.member.m_link);
            else
...
extract(file) register char *file;
{
    register int fd;
...
copy(file, from, to, bytes) char *file;
int from, to;
register long bytes;
{
    register int rest;
...
is_dir(file) register char *file;
{
    while (*file++ != '\0')
        ;
...
add_path(name) register char *name;
{
...
add_file(file) register char *file;
{
    struct stat st;
    struct direct dir;
    register int fd;
...
mread(fd, address, bytes) int fd, bytes;
char *address;
{
...
mwrite(fd, address, bytes) int fd, bytes;
char *address;
{
...
/* VARARGS */
string_print(buffer, fmt, args) char *buffer;
register char *fmt;
int args;
{
