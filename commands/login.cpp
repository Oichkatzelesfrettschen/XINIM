    ...
    public:
        ...
        static HashResult hash_password(std::string_view password) {
            ...
            if (PKCS5_PBKDF2_HMAC(password.data(), password.length(),
                                result.salt.data(), SALT_SIZE,
                                iterations, EVP_sha256(),
                                HASH_SIZE, result.hash.data()) != 1) {
        ...
        static bool verify_password(std::string_view password, const HashResult& stored) {
            ...
            const int iterations = 100000;
            if (PKCS5_PBKDF2_HMAC(password.data(), password.length(),
                                stored.salt.data(), SALT_SIZE,
                                iterations, EVP_sha256(),
                                HASH_SIZE, computed_hash.data()) != 1) {
                return false;
            }
...
public:
    ...
    void log_event(EventType type, const std::string& username, 
                   const std::string& source_ip = "127.0.0.1",
...
private:
    std::string event_type_string(EventType type) {
        switch (type) {
            case EventType::LOGIN_ATTEMPT: return "LOGIN_ATTEMPT";
            ...
            case EventType::PASSWORD_CHANGE: return "PASSWORD_CHANGE";
...
public:
    ...
    void reset_failures(const std::string& username) {
        failed_attempts_.erase(username);
    }
    
private:
    std::unordered_map<std::string, std::vector<std::chrono::steady_clock::time_point>> failed_attempts_;
};
...
private:
    struct Credentials {
        std::string username;
        secure_memory::SecureArray<char, 256> password;
    };
    ...
    bool authenticate_user(const Credentials& creds) {
        ...
        return crypto::constant_time_compare(result, encrypted_password, strlen(encrypted_password));
    }
