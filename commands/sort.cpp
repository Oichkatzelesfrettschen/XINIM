...
class ModernSortEngine {
  ...
  private:
    ...
    /// Merge multiple sorted input streams into the container.
    ///
    /// This performs a classic \em k-way merge using a min-heap. Each input
    /// file (and optionally standard input) is assumed to be individually
    /// sorted according to the configuration flags. The resulting merged
    /// sequence is appended to \p container while respecting the unique line
    /// option if enabled.
    auto merge_files(LineContainer &container) -> Result<void> {
        ...
        // Helper lambda to push a source into the heap when it has a line.
  ...
  public:
    ...
    [[nodiscard]] auto run() -> Result<void> {
        try {
            ...
            // Read input unless we are in merge mode (merge handles its own I/O)
