...
int next_zone, next_inode, zone_size, zone_shift = 0, zoff, nrblocks, inode_offset, nrinodes,
                                      lct = 1, disk, fd, print = 0, file = 0, override = 0,
                                      simple = 0;
...
super(zones, inodes) int zones, inodes;
{
    ...
    zo = 7L + (long)NR_INDIRECTS + (long)NR_INDIRECTS * NR_INDIRECTS;
...
eat_dir(parent) int parent; /* parent's inode nr */
{
...
eat_file(inode, f) int inode, f;
{
    ...
    do {
        ...
        if (ct)
            add_zone(inode, z, (long)j, file_time(f));
    } while (ct == BLOCK_SIZE);
    close(f);
}
...
enter_dir(parent, name, child) int parent, child; /* inode nums */
char *name;
{
...
add_zone(n, z, bytes, cur_time) int n, z;
long bytes, cur_time;
{
...
insert_bit(block, bit, count) int block, bit, count;
{
...
getline(line, parse) char *parse[MAX_TOKENS];
char line[LINE_LEN];
{
...
copy(from, to, count) char *from, *to;
int count;
{
    while (count--)
        *to++ = *from++;
    ...
    for (b = 4; b < 8; b++) {
        ...
        for (i = 0; i < INODES_PER_BLOCK; i++) {
            ...
            if (inode[i].i_mode != 0) {
                ...
                printf("  uid=%2d  gid=%2d  size=", inode[i].i_uid, inode[i].i_gid);
...
mx_read(blocknr, buf) int blocknr;
char buf[BLOCK_SIZE];
{
...
mx_write(blocknr, buf) int blocknr;
char buf[BLOCK_SIZE];
{
...
dexit(s, sectnum, err) int sectnum, err;
char *s;
{
