...
register struct wdblock **wbp;
{
    ...
    if (cp == NULL)
        return (0);
    if (!anys("$`'\"", cp) && !anys(ifs->value, cp) && ((f & DOGLOB) == 0 || !anys("[*?", cp))) {
...
loop:
    ...
    for (;;) {
        c = subgetc('"', 0);
        if (c == 0 || f & DOBLANK && any(c, ifs->value) || !INSUB() && any(c, "\"'`")) {
            unget(c);
            if (any(c, "\"'`"))
                goto loop;
            break;
