...
#define clus_add(cl_no) ((long)(((long)cl_no - 2L) * (long)cluster_size + (long)data_start))
struct dir_entry {
...
long mark;
short total_clusters, cluster_size, fat_size, root_entries, data_start, sub_entries;
...
int main(int argc, char *argv[]) {
    ...
    if (dos_dir) {
        ...
        if (*path)
            make_file(find_entry(root, root_entries, path), sub_entries, slash(argv[index]));
        else
...
static DIRECTORY *directory(DIRECTORY *dir, short entries, BOOL function, char *pathname) {
        ...
        for (i = 0; i < entries; i++, dir_ptr++) {
            ...
            if (function == ENTRY) {
                if (type == NOT_USED || type == ERASED) {
                    mark = lseek(disk, 0L, 1) - (long)cluster_size + (long)i * (long)DIR_SIZE;
                    if (!mem)
            ...
            if (type == DIR || type == ERASED || function == LABEL)
                continue;
            type = is_dir(dir_ptr);
            name = make_name(dir_ptr, (function == FIND) ? FALSE : type);
            if (function == FIND) {
                ...
                if (*pathname) {
                    dir_ptr = find_entry(dir_ptr, sub_entries, pathname + 1);
                }
                if (mem) {
                    if (dir_ptr) {
...
static void extract(DIRECTORY *entry) {
...
static void print(short fd, char *buffer, short bytes) {
    static short index;
...
static void make_file(DIRECTORY *dir_ptr, int entries, char *name) {
...
static void fill_date(DIRECTORY *entry) {
...
static char *make_name(DIRECTORY *dir_ptr, short dir_fl) {
...
{
    print_string(FALSE, "\t%c%c%c%c%c", (mode & SUB_DIR) ? 'd' : '-', (mode & 02) ? 'h' : '-',
                 (mode & 04) ? 's' : '-', (mode & 01) ? '-' : 'w', (mode & 0x20) ? 'a' : '-');
}

show(dir_ptr, name) DIRECTORY *dir_ptr;
char *name;
{
    ...
    print_string(FALSE, "\t%N:%N %P %s %d\n", ((e_time & HOUR) >> 11), ((e_time & MIN) >> 5),
                 (e_date & DAY), month[((e_date & MONTH) >> 5) - 1], ((e_date & YEAR) >> 9) + 1980);
}
...
static char *num_out(long number) {
...
/* VARARGS */
print_string(err_fl, fmt, args) BOOL err_fl;
char *fmt;
int args;
{
    char buf[200];
...
link_fat(cl_1, cl_2) unsigned short cl_1;
...
add_path(file, slash_fl) register char *file;
BOOL slash_fl;
{
...
bcopy(src, dest, bytes) register char *src, *dest;
short bytes;
{
    while (bytes--)
        *dest++ = *src++;
...
disk_io(op, seek, address, bytes) register BOOL op;
unsigned long seek;
DIRECTORY *address;
