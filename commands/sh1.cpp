 ...
 */
void setval(vp, val) struct var *vp;
char *val;
{
 ...
 */
void nameval(vp, val, name) register struct var *vp;
char *val, *name;
{
    register char *cp, *xp;
    char *nv;
...
void export(vp) struct var *vp;
{
    vp->status |= EXPORT;
}

void ronly(vp) struct var *vp;
{
...
void putvlist(f, out) register int f, out;
{
    register struct var *vp;
...
void freearea(a) register int a;
{
...
void garbage() {
    ...
    if (areatop >= q + nu && q->area > areanum) {
        brk((char *)(areabrk -= nu));
