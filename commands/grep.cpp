 ...
 * outputs matching lines. The modern implementation uses the C++ <regex>
 * library for robust pattern matching and modern I/O streams for file handling.
 *
 * Usage: grep [-vnse] pattern [file...]
...
private:
    ...
    bool process_stream(std::istream& stream, const std::string& filename) {
        ...
        while (std::getline(stream, line)) {
            ...
            bool should_print = (matches && !options_.invert_match) || 
                               (!matches && options_.invert_match);
    ...
    while (i < argc && argv[i][0] == '-' && argv[i][1] != '\0') {
        ...
        for (size_t j = 1; j < arg.length(); ++j) {
            switch (arg[j]) {
                ...
                case 's': opts.suppress_output = true; break;
                case 'e': 
