 ...
 */
LOW() { move_to(0, last_y); }

/*
 * Move to begin of line.
 */
BL() { move_to(LINE_START, y); }

/*
 * Move to end of line.
 */
EL() { move_to(LINE_END, y); }
 ...
 */
MP() { move_previous_word(NO_DELETE); }

move_previous_word(remove) FLAG remove;
{
 ...
 */
MN() { move_next_word(NO_DELETE); }

move_next_word(remove) FLAG remove;
{
 ...
 */
insert(line, location, string) register LINE *line;
 ...
 */
delete (start_line, start_textp, end_line, end_textp) register LINE *start_line;
LINE *end_line;
 ...
 */
set_up(remove) FLAG remove; /* DELETE if text should be deleted */
{
    FLAG checkmark();
 ...
 * the right order)
 */
yank(start_line, start_textp, end_line, end_textp, remove) LINE *start_line, *end_line;
char *start_textp, *end_textp;
...
scratch_file(mode) FLAG mode; /* Can be READ or WRITE permission */
{
 ...
 */
SF() { search("Search forward:", FORWARD); }

/*
 * SF searches backwards for an expression.
 */
SR() { search("Search reverse:", REVERSE); }
 ...
 * of the file.
 */
GR() { change("Global replace:", VALID); }

/*
 * LR() replaces all matches on the current line.
 */
LR() { change("Line replace:", NOT_VALID); }
 ...
 */
change(message, file) char *message; /* Message to prompt for expression */
FLAG file;
{
    ...
    do {
        if (line_check(program, line->text, FORWARD)) {
            ...
            do {
                subs++; /* Increment subs */
                if ((textp = substitute(line, program, replacement)) == NIL_PTR)
    ...
    if (subs == 0L && quit == FALSE)
        ...
        build_string(mess_buf, "%s %D substitutions on %D lines.", mess_buf, subs, lines);
 ...
 */
search(message, method) char *message;
FLAG method;
{
 ...
 */
find_y(match_line) LINE *match_line;
{
    register LINE *line;
    register int count = 0;

    /* Check if match_line is on the same page as currently displayed. */
    for (line = top_line; line != match_line && line != bot_line->next; line = line->next)
        count++;
 ...
 */
#define reg_error(str) program->status = REG_ERROR, program->result.err_mess = (str)
/*
 ...
 * this field.
 */
finished(program, last_exp) register REGEX *program;
int *last_exp;
{
 ...
 */
bcopy(from, to, bytes) register char *from, *to;
register int bytes;
{
    while (bytes--)
        *to++ = *from++;
 ...
 */
compile(pattern, program) register char *pattern; /* Pointer to pattern */
REGEX *program;
{
 ...
 */
line_check(program, string, method) register REGEX *program;
char *string;
FLAG method;
{
 ...
 */
check_string(program, string, expression) REGEX *program;
...
int *expression;
{
    ...
    while (quit == FALSE && !(*expression & DONE) && *string != '\0' && *string != '\n') {
 ...
 */
star(program, end_position, string, expression) REGEX *program;
 ...
 */
in_list(list, c, list_length, opcode) register int *list;
char c;
