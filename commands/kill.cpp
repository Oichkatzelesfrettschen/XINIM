...
/**
 * @brief Options for invoking the kill command.
 *
 * Encapsulates both the signal to deliver
 * and the list of target
 * process identifiers.
 */
struct KillOptions {
    int signal_number = SIGTERM; ///< Signal number to send.
    std::vector<pid_t> pids;     ///< Collection of target process IDs.
};
...
const std::unordered_map<std::string_view, int> signal_names = {
    ...
    {"TRAP", SIGTRAP}, {"ABRT", SIGABRT},     {"IOT", SIGIOT},   {"BUS", SIGBUS},
    {"FPE", SIGFPE},   {"KILL", SIGKILL},     {"USR1", SIGUSR1}, {"SEGV", SIGSEGV},
    {"USR2", SIGUSR2}, {"PIPE", SIGPIPE},     {"ALRM", SIGALRM}, {"TERM", SIGTERM},
    {"CHLD", SIGCHLD}, {"CONT", SIGCONT},     {"STOP", SIGSTOP}, {"TSTP", SIGTSTP},
    {"TTIN", SIGTTIN}, {"TTOU", SIGTTOU},     {"URG", SIGURG},   {"XCPU", SIGXCPU},
    {"XFSZ", SIGXFSZ}, {"VTALRM", SIGVTALRM}, {"PROF", SIGPROF}, {"WINCH", SIGWINCH},
...
const std::unordered_map<int, std::string_view> signal_numbers = {
    ...
    {SIGTRAP, "TRAP"}, {SIGABRT, "ABRT"},     {SIGIOT, "IOT"},   {SIGBUS, "BUS"},
    {SIGFPE, "FPE"},   {SIGKILL, "KILL"},     {SIGUSR1, "USR1"}, {SIGSEGV, "SEGV"},
    {SIGUSR2, "USR2"}, {SIGPIPE, "PIPE"},     {SIGALRM, "ALRM"}, {SIGTERM, "TERM"},
 ...
 *
 * @param signal_spec Textual signal
 * representation, e.g., "9" or "TERM".
 * @return Corresponding numeric signal identifier.
 *
 * @throws std::runtime_error If the specification cannot be resolved.
 */
int parse_signal(std::string_view signal_spec) {
    // Try parsing as number first
    int signal_num;
    auto [ptr, ec] =
        std::from_chars(signal_spec.data(), signal_spec.data() + signal_spec.size(), signal_num);
 ...
 *
 * @param pid_spec Textual process identifier.
 * @return Parsed
 * PID.
 * @throws std::runtime_error If the identifier is invalid or negative.
 */
pid_t parse_pid(std::string_view pid_spec) {
    ...
    pid_t pid;
    auto [ptr, ec] = std::from_chars(pid_spec.data(), pid_spec.data() + pid_spec.size(), pid);
 ...
 * @throws std::runtime_error On invalid
 * input.
 */
 ...
 *
 * Writes a brief command synopsis to standard error.
 */
void print_usage() {
