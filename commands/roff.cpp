...
int o_ta[20] = {9,  17, 25,  33,  41,  49,  57,  65,  73,  81,
...
char *sprintf();
char *request[] = {"ad", "ar", "bl", "bp", "br", "cc", "ce", "de", "ds", "ef", "eh", "fi", "fo",
                   "hc", "he", "hx", "hy", "ig", "in", "ix", "li", "ll", "ls", "m1", "m2", "m3",
                   "m4", "n1", "n2", "na", "ne", "nf", "ni", "nn", "nx", "of", "oh", "pa", "pl",
                   "po", "ro", "sk", "sp", "ss", "ta", "tc", "ti", "tr", "ul", 0};
...
writeline(adflag, flushflag) int adflag, flushflag;
{
    int j, q;
    char lnstring[7];
    for (j = assylen - 1; j; j--) {
...
spit(c) char c;
{
    ...
    if (page_no < startpage || (stoppage && page_no > stoppage))
        return;
    if (c != ' ' && c != '\n' && n_blanks) {
        if (hflag && n_blanks > 1)
 ...
 *	We could just as well use <ctype.hpppp> if it didn't vary from
