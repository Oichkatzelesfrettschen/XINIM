...
#define library(nm)                                                                                \
    mkstr(alloc((unsigned int)strlen(nm) + strlen(LIBDIR) + 7), LIBDIR, "/lib", nm, ".a", 0)
...
/*VARARGS1*/
char *static char *mkstr(char *dst, char *arg) {
    char **vec = (char **)&arg;
    register char *p;
    register char *q = dst;
...
static void basename(char *str, char *dst) {
...
int static int extension(char *fn) {
    char c;
...
static int runvec2(struct arglist *vec0, struct arglist *vec1) {
    /* set up 'vec0 | vec1' */
...
/*VARARGS1*/
static void panic(char *str, int argv) {
    write(2, str, strlen(str));
    exit(1);
}

char *static char *cindex(char *s, char c) {
    while (*s)
        if (*s++ == c)
            return s - 1;
    return (char *)0;
...
static void pr_vec(struct arglist *vec) {
...
static void ex_vec(struct arglist *vec) {
    extern int errno;
...
static void mktempname(char nm[]) {
    register i;
    int pid = getpid();
