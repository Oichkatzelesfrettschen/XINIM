 ...
 *
 * Channel exhaustion returns an error. Message source tagging and blocked
 * sender/receiver tracking provide queue back-pressure, while an incoming
 * channel bitset accelerates receive lookup.
 */
...
namespace lattice {
    ...
    int lattice_connect(xinim::pid_t src, xinim::pid_t dst, net::node_t node_id) {
        ...
        if (!ch1)
            return static_cast<int>(ErrorCode::E_CHAN_FULL);
    ...
    int lattice_send(xinim::pid_t src, xinim::pid_t dst, const message &msg,
        ...
        // Tag the source at the delivery boundary.
        message tagged = msg;
        tagged.m_source = src;
        ...
        // Block the sender until queue space opens.
        auto *pcb = xinim::kernel::g_unified_scheduler.find_by_pid(src);
        if (pcb) {
            ch->add_blocked_sender(src);
            xinim::kernel::g_unified_scheduler.block(pcb, xinim::kernel::BlockReason::IPC_SEND,
                                                     dst);
        }
    ...
    int lattice_recv(xinim::pid_t pid, message *out, [[maybe_unused]] IpcFlags flags) {
        ...
        // Scan only channels marked as having pending input.
        ...
        // Block the receiver until a message arrives.
        auto *pcb = xinim::kernel::g_unified_scheduler.find_by_pid(pid);
        if (pcb) {
            g_graph.set_listening(pid, true);
            xinim::kernel::g_unified_scheduler.block(pcb, xinim::kernel::BlockReason::IPC_RECV, -1);
        }
        return static_cast<int>(ErrorCode::E_NO_MESSAGE);
    }

    void poll_network() {
        // No transport backend is bound; local IPC uses the in-memory graph.
    }
