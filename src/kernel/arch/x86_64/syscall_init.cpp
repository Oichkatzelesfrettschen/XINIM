 ...
 * @brief Syscall initialization implementation
 *
...
namespace xinim::kernel {
    ...
    [[maybe_unused]] constexpr uint32_t MSR_CSTAR =
        0xC0000083;                            ///< Compatibility Mode Syscall Target
    ...
    constexpr uint64_t RFLAGS_DF = (1ULL << 10); ///< Direction Flag
    ...
    void initialize_syscall() {
        ...
        // Program IA32_STAR (Syscall Target Address Register).
        ...
        // Program IA32_LSTAR (Long Mode Syscall Target Address).
        ...
        snprintf(buffer, sizeof(buffer), "[SYSCALL] Set IA32_LSTAR: handler=0x%lx\n", handler_addr);
        early_serial.write(buffer);

        // Program IA32_FMASK (Syscall Flag Mask).
        // Interrupts stay disabled until IRETQ. Clearing DF also guarantees the
        // language ABI before the assembly entry executes its defensive CLD.
        wrmsr(MSR_FMASK, RFLAGS_IF | RFLAGS_DF);

        early_serial.write("[SYSCALL] Set IA32_FMASK: will clear IF and DF\n");

        // Verify that IA32_EFER retained the SCE bit.
