...
namespace xinim::kernel::bootfs {
    ...
    inline constexpr uint32_t O_APPEND = 0x0400U;
    ...
    inline constexpr uint32_t kPipeBufferSize = 4096U;
#if defined(XINIM_ARCH_X86_64)
    inline constexpr size_t kMaximumDynamicNodes = xinim::limits::MAX_PROCESS_COUNT;
#else
    inline constexpr size_t kMaximumDynamicNodes = 16U;
#endif
    inline constexpr size_t kMaximumSymbolicLinks = 64U;
    inline constexpr size_t kStaticFileRecordCapacity = 128U;
    inline constexpr size_t kMaximumFileRecords =
        kStaticFileRecordCapacity + kMaximumDynamicNodes + kMaximumSymbolicLinks;
    inline constexpr size_t kMaximumBackendUserOpenFiles = 29U;
    ...
    struct FileRecord {
        ...
        uint32_t user_id;
        uint32_t group_id;
        uintptr_t exclusive_lock_owner;
        uint32_t shared_lock_count;
    };
    ...
    inline constexpr int kLockWouldBlock = -4;
    ...
    [[nodiscard]] bool can_read_without_block(int fd) noexcept;
    [[nodiscard]] bool can_write_without_block(int fd) noexcept;
