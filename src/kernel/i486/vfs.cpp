...
VfsOps* resolve(const char* path, const char** relative_path) noexcept {
    ...
    for (auto& m : g_mounts) {
        ...
        if (!prefix_match(path, m.prefix, m.prefix_len)) {
            continue;
        }
...
int mount_index(const char* path) noexcept {
    ...
    for (int i = 0; i < kMaxMounts; ++i) {
        ...
        if (!prefix_match(path, g_mounts[i].prefix, g_mounts[i].prefix_len)) {
            continue;
        }
