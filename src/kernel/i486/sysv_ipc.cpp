...
int sys_shmget(key_t key, uint32_t size, int shmflg) noexcept {
    ...
    for (uint32_t i = 0U; i < kMaxShmSegments; ++i) {
        if (!g_shm[i].in_use) {
            ...
            for (uint32_t j = 0U; j < kMaxShmSize; ++j) {
                g_shm[i].data[j] = 0U;
            }
            return static_cast<int>(i);
        }
...
int sys_shmat(int shmid, uint32_t /*shmaddr*/, int /*shmflg*/,
    ...
    if (shmid < 0 || static_cast<uint32_t>(shmid) >= kMaxShmSegments) {
        return -22;
    }
    if (!g_shm[shmid].in_use) {
        return -22;
    }
...
int sys_semget(key_t key, int nsems, int semflg) noexcept {
    if (nsems < 0 || static_cast<uint32_t>(nsems) > kMaxSemsPerSet) {
        return -22;
    }
...
int sys_semop(int semid, const SemBuf* sops, uint32_t nsops) noexcept {
    if (semid < 0 || static_cast<uint32_t>(semid) >= kMaxSemSets) {
        return -22;
    }
    if (!g_sem[semid].in_use || sops == nullptr) {
        return -22;
    }
    ...
    for (uint32_t i = 0U; i < nsops; ++i) {
        ...
        if (sops[i].sem_op < 0 && val < -sops[i].sem_op) {
            if (sops[i].sem_flg & static_cast<int16_t>(IPC_NOWAIT)) {
                return -11; // EAGAIN
            }
...
int sys_semctl(int semid, int semnum, int cmd, int val) noexcept {
    ...
    case SETVAL:
        if (semnum < 0 || static_cast<uint32_t>(semnum) >= set.ds.sem_nsems) {
            return -22;
        }
...
int sys_msgsnd(int msqid, const void* msgp, uint32_t msgsz, int msgflg) noexcept {
    if (msqid < 0 || static_cast<uint32_t>(msqid) >= kMaxMsgQueues) {
        return -22;
    }
    if (!g_msg[msqid].in_use || msgp == nullptr) {
        return -22;
    }
    if (msgsz > kMaxMsgSize) {
        return -22;
    }
