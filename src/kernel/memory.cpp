...
extern "C" void mem_task() noexcept {
    ...
    while (TRUE) {
        ipc_receive(ANY, &mess);
        if (mess.m_source < 0)
            kpanic("mem task got bad message source");
...
[[nodiscard]] static int do_mem(message *m_ptr) noexcept {
    ...
    if (mem_phys >= ram_limit[minor_index])
        return (EOF);
    ...
    if (m_ptr->m_type == DISK_READ) {
        phys_copy(reinterpret_cast<void *>(static_cast<uintptr_t>(user_phys)),
                  reinterpret_cast<const void *>(static_cast<uintptr_t>(mem_phys)), byte_count);
    } else {
        phys_copy(reinterpret_cast<void *>(static_cast<uintptr_t>(mem_phys)),
                  reinterpret_cast<const void *>(static_cast<uintptr_t>(user_phys)), byte_count);
    }
...
[[nodiscard]] static int do_setup(message *m_ptr) noexcept {
    ...
    ram_limit[minor_index] =
        static_cast<uint64_t>(position(*m_ptr)) +
