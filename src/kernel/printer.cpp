...
class PrinterDevice {
public:
    void initialize(int base) noexcept {
        base_ = base;
...
static PrinterDevice g_printer_device;
...
static void do_write(message *m_ptr) noexcept {
    ...
    r = OK;
    if (pr_busy)
        r = static_cast<int>(ErrorCode::EAGAIN);
    if (count(*m_ptr) <= 0)
    ...
    if (phys == 0)
        r = static_cast<int>(ErrorCode::E_BAD_ADDR);
...
static void do_done(message *m_ptr) noexcept {
    int status =
        (rep_status(*m_ptr) == OK ? static_cast<int>(o_count) : static_cast<int>(ErrorCode::EIO));
...
static void pr_error(int status) noexcept {
    if (status & NO_PAPER)
        Console::printf("Printer is out of paper\n");
    if ((status & OFF_LINE) == 0)
        Console::printf("Printer is not on line\n");
    if ((status & PR_ERR_BIT) == 0)
        Console::printf("Printer error\n");
}

static void print_init() noexcept {
    extern int color;
    g_printer_device.initialize(color ? PR_COLOR_BASE : PR_MONO_BASE);
    pr_busy = FALSE;
}
