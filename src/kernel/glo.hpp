...
#include <sys/const.hpp> // For NR_TASKS and other global constants
#include <sys/type.hpp>  // For real_time, message, xinim::pid_t (via core_types.hpp)
#include "./const.hpp"    // For TASK_STACK_BYTES, K_STACK_BYTES

/* Clocks and timers */
EXTERN real_time realtime; /* real time clock (real_time -> xinim::time_t) */
EXTERN real_time
    lost_ticks; /* incremented when clock int can't send mess, (real_time -> xinim::time_t) */
