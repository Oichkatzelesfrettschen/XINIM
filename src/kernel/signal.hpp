...
#define SIG_SETMASK 2 ///< Set mask to given value
    ...
    struct SignalState {
        ...
        bool suspend_active;          ///< rt_sigsuspend installed a temporary mask.
        uint64_t suspend_saved_mask;  ///< Mask restored after sigsuspend delivery.
    };
     ...
     */
    [[nodiscard]] bool init_signal_state(ProcessControlBlock *pcb) noexcept;

    /** Release the signal state owned by a process. */
    void destroy_signal_state(ProcessControlBlock *pcb) noexcept;

    /** Copy dispositions and the blocked mask across fork. */
    [[nodiscard]] bool copy_signal_state(const ProcessControlBlock *parent,
     ...
     */
    [[nodiscard]] int take_pending_signal(ProcessControlBlock *pcb) noexcept;
