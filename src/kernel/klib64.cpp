...
// The free-list heap reserves four MiB and returns aligned reusable blocks.
...
void *mmap(void *, size_t, int, int, int, std::int64_t) {
    return (void *) -1;
}
...
void phys_copy(void *dst, const void *src, size_t n) noexcept {
    ...
    size_t n_out = n;
    asm volatile("rep movsb" : "+S"(src_out), "+D"(dst_out), "+c"(n_out) : : "memory");
#else
    memcpy(dst, src, n);
#endif
}
...
void phys_copy16(void *dst, const void *src, size_t words) noexcept {
    ...
    asm volatile("rep movsw" : "+S"(src_out), "+D"(dst_out), "+c"(words_out) : : "memory");
#else
...
void cp_mess(int src_proc_nr, uint64_t, const void *src_payload, uint64_t,
             void *dst_payload) noexcept {
