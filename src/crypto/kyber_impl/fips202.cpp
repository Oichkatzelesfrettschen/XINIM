...
namespace xinim::crypto::fips202 {
    ...
    static constexpr std::array<std::size_t, 25> keccak_piln = {{
    ...
    static void keccakf1600(std::array<std::uint64_t, 25> &st) noexcept {
        for (std::size_t round = 0U; round < keccak_rc.size(); ++round) {
            // Theta
            std::array<std::uint64_t, 5> column_parity{};
            for (std::size_t column = 0U; column < column_parity.size(); ++column) {
                column_parity[column] = st[column] ^ st[column + 5U] ^ st[column + 10U] ^
                                        st[column + 15U] ^ st[column + 20U];
            }

            for (std::size_t column = 0U; column < column_parity.size(); ++column) {
                const std::uint64_t theta_delta = column_parity[(column + 4U) % 5U] ^
                                                  std::rotl(column_parity[(column + 1U) % 5U], 1);
                for (std::size_t row_offset = 0U; row_offset < st.size(); row_offset += 5U) {
                    st[row_offset + column] ^= theta_delta;
                }
            }

            // Rho and Pi
            std::array<std::uint64_t, 25> permuted{};
            for (std::size_t lane = 0U; lane < st.size(); ++lane) {
                permuted[keccak_piln[lane]] = std::rotl(st[lane], keccak_rotc[lane]);
            }

            // Chi
            for (std::size_t row_offset = 0U; row_offset < st.size(); row_offset += 5U) {
                for (std::size_t column = 0U; column < 5U; ++column) {
                    st[row_offset + column] = permuted[row_offset + column] ^
                                              (~permuted[row_offset + (column + 1U) % 5U] &
                                               permuted[row_offset + (column + 2U) % 5U]);
                }
            }
     ...
     */
    static void keccak_absorb(std::array<std::uint64_t, 25> &s, std::size_t rate,
    ...
    static void keccak_squeeze(std::uint8_t *out, std::size_t outlen,
                               std::array<std::uint64_t, 25> &s, std::size_t rate) noexcept {
        std::size_t rate_bytes = rate;
     ...
     */
    static void keccak_absorb_once(keccak_state &state, std::size_t rate, const std::uint8_t *in,
                                   std::size_t inlen, std::uint8_t dsb) noexcept {
        state.s = {};
        state.pos = 0;
    ...
    public:
        const char *name() const noexcept override { return "fips202"; }
    ...
    fips202_result shake128_absorb(keccak_state &state, std::span<const std::uint8_t> in) noexcept {
        keccak_absorb_once(state, SHAKE128_RATE, in.data(), in.size(), 0x1F);
        return {};
    }

    fips202_result shake256_absorb(keccak_state &state, std::span<const std::uint8_t> in) noexcept {
    ...
    fips202_result shake256_squeeze(std::span<std::uint8_t> out, keccak_state &state) noexcept {
        if (out.empty()) {
            return std::unexpected(make_error_code(fips202_error::invalid_output_length));
        }
    ...
    std::expected<std::vector<std::uint8_t>, error_code> shake128(std::span<const std::uint8_t> in,
    ...
    std::expected<std::vector<std::uint8_t>, error_code> shake256(std::span<const std::uint8_t> in,
                                                                  std::size_t output_length) {
        if (output_length == 0) {
            return std::unexpected(make_error_code(fips202_error::invalid_output_length));
        }
