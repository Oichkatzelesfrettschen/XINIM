...
namespace {
    ...
    [[nodiscard]] std::expected<Options, std::string> parse_arguments(std::span<char *> args) {
        Options opts;
        ...
        for (size_t i = 0; i < args.size(); ++i) {
            ...
            if (parsing_options && arg.starts_with("-")) {
                if (arg == "-i" || arg == "--ignore-environment") {
                    ...
                    return std::unexpected(std::format("env: invalid option: {}", arg));
                }
            } else if (arg.contains('=')) {
                // Variable assignment
                auto pos = arg.find('=');
                opts.vars[std::string(arg.substr(0, pos))] = std::string(arg.substr(pos + 1));
            } else {
                // Start of command
                parsing_options = false;
        ...
        if (environ) {
            for (char **p = environ; *p; ++p) {
                ...
                if (auto pos = entry.find('='); pos != std::string_view::npos) {
                    env[std::string(entry.substr(0, pos))] = std::string(entry.substr(pos + 1));
                }
            }
        }
...
// Platform-specific setenv (we'll use putenv as fallback)
    ...
    void print_environment(const std::map<std::string, std::string> &env, bool null_terminated) {
...
int main(int argc, char *argv[]) {
    ...
    const std::span<char *> process_arguments(argv, argument_count);
    const std::span<char *> utility_arguments =
        process_arguments.subspan(argument_count == 0U ? 0U : 1U);

    auto parse_result = parse_arguments(utility_arguments);
    if (!parse_result) {
    ...
    auto env = opts.ignore_environment ? std::map<std::string, std::string>{} : get_environment();
