...
namespace {
    ...
    private:
         ...
         */
        void output(std::size_t column, const std::string &line);
     ...
     */
    void FileComparer::output(std::size_t column, const std::string &line) {
        bool suppress = false;
        switch (column) {
        case 1U:
        ...
        if (!suppress) {
            std::cout << m_tabs[column - 1U] << line << '\n';
        }
    }
