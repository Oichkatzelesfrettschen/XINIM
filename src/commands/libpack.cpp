 ...
 */
const std::array<std::string_view, 128> compression_table = {"push ax",
                                                             "ret",
                                                             "mov bp,sp",
                                                             "push bp",
                                                             "pop bp",
...
class CompressionEngine {
  public:
    /**
     * @brief Construct a new CompressionEngine and initialize lookup data.
     */
    CompressionEngine() { build_lookup_map(); }
  ...
  private:
    /**
     * @brief Populate the pattern lookup map and determine the longest pattern length.
 */
    ...
    /// @brief Maps patterns to token indices for fast lookup.
    std::unordered_map<std::string_view, size_t> pattern_to_index_;

    /// @brief Length of the longest pattern in the compression table.
...
void print_statistics(size_t original_size, size_t compressed_size) {
    if (original_size > 0) {
        ...
        std::cerr << "Compression ratio: " << std::fixed << std::setprecision(1) << ratio << "%"
                  << std::endl;
    }
}
...
int main(int argc, char *argv[]) {
    try {
        ...
        if (decompress_mode) {
            ...
            if (verbose) {
                std::cerr << "Decompressed " << compressed_data.size() << " bytes to "
