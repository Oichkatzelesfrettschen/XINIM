 ...
 *
 * Implements directory listing with C++23 resource management and typed
 * filesystem metadata.
 */
...
namespace xinim::commands::ls {
    ...
    namespace {
         ...
         */
        mode_t to_posix_mode_from_fs_perms(
    ...
    namespace simd_ops {
         ...
         */
        [[nodiscard]] inline int compare_strings_simd(std::string_view lhs,
    ...
    public:
         ...
         */
        [[nodiscard]] const TimePoint &modification_time() const noexcept {
    ...
    private:
        ...
        static constexpr std::array setuid_strings{
            "---"sv, "--x"sv, "-w-"sv, "-wx"sv, "r--"sv, "r-x"sv, "rw-"sv, "rwx"sv,
            "--S"sv, "--s"sv, "-wS"sv, "-ws"sv, "r-S"sv, "r-s"sv, "rwS"sv, "rws"sv};
        static constexpr std::array setgid_strings{
            "---"sv, "--x"sv, "-w-"sv, "-wx"sv, "r--"sv, "r-x"sv, "rw-"sv, "rwx"sv,
            "--S"sv, "--s"sv, "-wS"sv, "-ws"sv, "r-S"sv, "r-s"sv, "rwS"sv, "rws"sv};
        static constexpr std::array sticky_strings{
            "---"sv, "--x"sv, "-w-"sv, "-wx"sv, "r--"sv, "r-x"sv, "rw-"sv, "rwx"sv,
            "--T"sv, "--t"sv, "-wT"sv, "-wt"sv, "r-T"sv, "r-t"sv, "rwT"sv, "rwt"sv};
    ...
    private:
        using UserId = FileInfo::UserId;
        using GroupId = FileInfo::GroupId;

        mutable std::unordered_map<UserId, std::string> uid_cache_;
        mutable std::unordered_map<GroupId, std::string> gid_cache_;
        mutable std::optional<std::ifstream> passwd_file_;
        mutable std::optional<std::ifstream> group_file_;

    public:
        [[nodiscard]] std::optional<std::string> get_username(UserId user_id) const {
            if (auto it = uid_cache_.find(user_id); it != uid_cache_.end())
                return it->second;
            return load_username(user_id);
        }
        [[nodiscard]] std::optional<std::string> get_groupname(GroupId group_id) const {
            if (auto it = gid_cache_.find(group_id); it != gid_cache_.end())
                return it->second;
            return load_groupname(group_id);
        }

    private:
        std::optional<std::string> load_username(UserId user_id) const {
            try {
                if (!passwd_file_) {
                ...
                while (std::getline(*passwd_file_, line)) {
                    ...
                    if (tokens.size() >= 3) {
                        try {
                            const unsigned long parsed_id = std::stoul(tokens[2]);
                            if (parsed_id <= std::numeric_limits<UserId>::max() &&
                                static_cast<UserId>(parsed_id) == user_id) {
                                uid_cache_[user_id] = tokens[0];
                                return tokens[0];
                            }
        ...
        std::optional<std::string> load_groupname(GroupId group_id) const {
            try {
                if (!group_file_) {
                ...
                while (std::getline(*group_file_, line)) {
                    ...
                    if (tokens.size() >= 3) {
                        try {
                            const unsigned long parsed_id = std::stoul(tokens[2]);
                            if (parsed_id <= std::numeric_limits<GroupId>::max() &&
                                static_cast<GroupId>(parsed_id) == group_id) {
                                gid_cache_[group_id] = tokens[0];
                                return tokens[0];
                            }
                        } catch (const std::exception &) {
                            continue;
        ...
        tokenize_passwd_line(const std::string &line) {
    ...
    private:
        ...
        void process_single_path(const std::string &path_str) {
            ...
            try {
                ...
                // The original code added it to files_ and then called
                // sort_files_and_print_listing. This behavior is kept.
                files_.emplace_back(std::move(initial_file_info));
                sort_files_and_print_listing();
                return;
            }

            if (initial_file_info.is_directory() &&
            ...
            for (const auto &path_str : paths) {
                ...
                try {
                    ...
                    std::println(std::cerr, "ls: cannot access '{}': {} ({})", path_str, e.what(),
                                 e.code().message());
                    overall_exit_status_ = 1; // EXIT_FAILURE
                    // If stat fails for a command line arg, it's often still listed (e.g., with ???
                    // for details). The original code implicitly did this by continuing. We'll add
                    // it to file_args to maintain this.
                }
                // Add to appropriate list even if stat failed, so it can be reported as "cannot
                // access"
        ...
        [[nodiscard]] static constexpr bool has_flag(ListingFlags flags,
                                                     ListingFlags flag) noexcept {
        ...
        void stat_file(FileInfo &file_info) {
            ...
            bool is_symlink_on_path = std::filesystem::is_symlink(
                std::filesystem::symlink_status(p, ec_is_symlink_check));

            if (is_symlink_on_path) {
                if (has_flag(flags_, ListingFlags::LongFormat) ||
                    has_flag(flags_, ListingFlags::DirectoryOnly)) {
                    ctx.follow_symlinks = false; // For `ls -l symlink` or `ls -d symlink`, show
        ...
        void expand_directory_impl(const std::string &directory_path) {
            try {
                ...
                for (const auto &entry : std::filesystem::directory_iterator(directory_path)) {
                    ...
                    files_.emplace_back(
                        filename); // Use relative filename for listing within directory
                    ...
                    if (should_stat_file()) {
                        ...
                        // Otherwise (not -l), we effectively "follow" it by statting the target if
                        // it's a directory entry. std::filesystem::directory_iterator does not
                        // follow symlinks itself. Here, we check if the entry is a symlink. If so,
                        // and -l, don't follow.
                        ...
                        if (status_result) {
                            ...
                            // Only print error if it's not a hidden file or if hidden files are
                            // shown
                            if (!filename.starts_with('.') ||
                                has_flag(flags_, ListingFlags::ShowAll)) {
                                std::println(std::cerr, "ls: cannot access '{}': {}",
                                             full_entry_path.string(),
        ...
        void sort_files() {
             ...
             */
            auto get_file_ref = [&](std::size_t index) -> const FileInfo & {
                return files_[index];
            };
            if (has_flag(flags_, ListingFlags::SortByTime)) {
                ...
                const auto name_comparator = [&](std::size_t a, std::size_t b) {
                    const auto cmp_result = simd_ops::compare_strings_simd(get_file_ref(a).name(),
                                                                           get_file_ref(b).name());
        ...
        void print_total_blocks() const {
            ...
            const auto total_blocks =
                std::accumulate(sort_indices_.begin(), sort_indices_.end(), 0ULL,
                                /**
                                ...
                                [&](std::uintmax_t sum, std::size_t index) {
                                    ...
                                    // Only count blocks for files/dirs that were successfully
                                    // stat'd. For -d on a directory, its own size might be listed
                                    // as blocks. Standard ls usually shows 0 for directories unless
                                    // -s on directory argument. This logic is simplified: if
                                    // stat'd, and not a directory OR -d is set, count its blocks.
        ...
        void print_file_line(const FileInfo &file_info) const {
            ...
            if (has_flag(flags_, ListingFlags::ShowInodes)) {
                line_buffer += std::format("{:5} ", file_info.is_stat_performed()
                                                        ? std::to_string(file_info.inode())
                                                        : "?"s);
            }
            ...
            if (has_flag(flags_, ListingFlags::LongFormat)) {
                if (file_info.is_stat_performed()) {
                    ...
                    line_buffer += "-????????? ? ?        ?             ? ";
                } // Unknown fields preserve the long-listing column layout after a stat error.
            }
        ...
        [[nodiscard]] std::string get_long_format_string(const FileInfo &file_info) const {
            ...
            part +=
                std::format("{} {:2} ", PermissionFormatter::format_permissions(file_info.mode()),
            ...
            if (file_info.is_device())
                part += std::format(
                    "{:3}, {:3} ", (file_info.size() >> 8) & 0xFF,
                    file_info.size() & 0xFF); // Major/minor from rdevice stored in size for devices
            else
        ...
        get_formatted_time_string(const std::chrono::system_clock::time_point &time) const {
            ...
            // but for ls, which is typically single-threaded per invocation, std::localtime is
            // common. For robust C++20/23, std::chrono::format could be used if available and
            // suitable.
            ...
            if (tm_info) {
                ...
                time_str += month_names[static_cast<std::size_t>(tm_info->tm_mon)];
