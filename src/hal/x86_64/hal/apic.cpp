...
namespace xinim::hal::x86_64 {
    ...
    void Lapic::init(uintptr_t mmio_base) {
        ...
        mmio_write(base_, 0xF0, static_cast<uint32_t>(kSpuriousVector) | (1U << 8U));
    }
    ...
    void Lapic::setup_timer(uint8_t vector, uint32_t initial_count, uint8_t divide_power_of_two,
        ...
        uint32_t div = 0x3;
        if (divide_power_of_two == 1)
            div = 0x0;
        else if (divide_power_of_two == 2)
            div = 0x1;
        else if (divide_power_of_two == 3)
            div = 0x2;
        else if (divide_power_of_two == 4)
            div = 0x3; // 16
