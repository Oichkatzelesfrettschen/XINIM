...
alignas(16) BootModule g_boot_modules[kMaxBootModules]{};
...
BootInfo from_multiboot2(uint32_t magic, uintptr_t info_addr) noexcept {
    ...
    while (cursor + sizeof(MultibootTag) <= end) {
        ...
        case kTagTypeCmdline:
            ...
            break;
        case kTagTypeModule: {
            if (info.modules_count >= kMaxBootModules) {
                break;
            }
            const auto* module_tag = reinterpret_cast<const MultibootTagModule*>(tag);
            BootModule& module = g_boot_modules[info.modules_count];
            module.address = reinterpret_cast<const void*>(static_cast<uintptr_t>(module_tag->mod_start));
        ...
        case kTagTypeMmap: {
            ...
            for (uintptr_t entry_addr = entries_begin;
                 entry_addr + sizeof(MultibootMmapEntry) <= entries_end && count < kMaxMemoryRanges;
        ...
        case kTagTypeFramebuffer: {
            ...
            info.framebuffer.address = reinterpret_cast<void*>(static_cast<uintptr_t>(fb_tag->address));
